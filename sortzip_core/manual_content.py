...
<tr><td><b>命名</b></td><td><b>排序设置：</b>7 种排序预设（文件名升降序、修改时间、文件大小、扩展名）；<b>保持原文件夹层级：</b>保留源目录结构，子文件夹独立重命名/压缩；每个文件夹独立命名模板 + 预设快速填充；压缩后重命名（启用后缀替换）</td></tr>
<tr><td><b>预览</b></td><td>标签页切换文件夹，查看所有文件重命名前后对比</td></tr>
<tr><td><b>设置</b></td><td>深色模式切换、执行前确认参数、使用手册、版本信息</td></tr>
...
<ul>
  ...
  <li>设置页「执行前确认参数」默认启用，每次执行前弹窗展示关键参数，确认后继续</li>
  <li>首次使用建议先小范围测试</li>
</ul>
"""
