...
<h3>工作流程概述</h3>
<ol>
  <li><b>设置输入与输出</b> — 在「文件」页面选择输入文件夹和输出文件夹</li>
  ...
  <li><b>查看结果</b> — 完成后弹出统计报告，可点击「打开输出文件夹」跳转</li>
</ol>
<p><b>执行前自动校验：</b>输出文件夹冲突、输入文件类型匹配、命名模板重复检测。</p>
...
<tr><td><b>文件</b></td><td><b>文件选择：</b>输入文件夹、输出文件夹、包含子文件夹；<b>密码设置：</b>压缩密码、确认密码、记住密码；<b>分卷设置：</b>每包文件数、分卷大小、启用分卷</td></tr>
<tr><td><b>映射</b></td><td>按类型分组勾选扩展名（视频/音乐/图片/文档/程序/字体/压缩包），支持自定义文件夹名，支持拖拽排序；<b>「映射全部」</b>一键扫描输入文件夹并启用全部文件类型的映射（预设外的扩展名弹窗输入文件夹名后自动创建），<b>「删除全部」</b>一键清空所有已启用映射</td></tr>
<tr><td><b>命名</b></td><td><b>排序设置：</b>7 种排序预设（文件名升降序、修改时间、文件大小、扩展名）；<b>保持原文件夹层级：</b>保留源目录结构，子文件夹独立重命名/压缩；每个文件夹独立命名模板 + 预设快速填充；命名规则表支持拖拽排序；压缩后重命名（一次/二次压缩后各自独立的后缀替换，一次压缩后仅限未启用分卷）</td></tr>
<tr><td><b>预览</b></td><td>标签页切换文件夹，查看所有文件重命名前后对比，支持拖拽手动排序（影响实际执行顺序）</td></tr>
<tr><td><b>设置</b></td><td>深色模式切换、执行前确认参数（含文件包含情况提示）、使用手册、版本信息、还原默认设置、自动更新检查</td></tr>
<tr><td><b>开始</b></td><td><b>压缩设置：</b>一次压缩、二次压缩、压缩格式、压缩后校验完整性；<b>其他选项：</b>保留原始文件、输出目录（生成 List.txt）、自动关闭 Bandizip；执行/取消、实时进度条与日志、完成后统计报告（含打开输出文件夹）</td></tr>
</table>
...
<ul>
  ...
  <li>开启「保持原文件夹层级」时，子文件夹自动继承父级命名规则（逐级向上查找<code>match_folder</code>）</li>
</ul>
...
<p>开启层级模式时，每个子文件夹独立重命名与压缩，生成独立压缩包和对照表。</p>
...
<ul>
  ...
  <li>勾选「保留原始文件」时输入文件仅复制，压缩后副本自动清理</li>
  <li>取消勾选「一次压缩」跳过所有压缩，仅分类与重命名</li>
  <li>取消勾选「启用分卷」强制单文件输出，不进行分卷</li>
  <li>开启「保持原文件夹层级」需同时勾选「包含子文件夹」</li>
  <li>层级模式下，子文件夹自动继承父级命名规则，每个子文件夹独立重命名和压缩</li>
  <li>文件名排序使用自然序（<code>1,2,…,10,11</code>），非字典序</li>
  <li>未启用压缩时，List.txt 只输出三列（序号/原文件名/新文件名），不含「所属压缩包名」</li>
  <li>设置页「执行前确认参数」默认启用，每次执行前弹窗展示关键参数及文件包含情况，确认后继续</li>
  <li>扩展名映射表和命名规则表均支持拖拽整行调整顺序</li>
  <li>「映射全部」扫描输入文件夹（遵循「包含子文件夹」设置）的全部文件类型并启用映射，预设外的扩展名会弹窗提示输入文件夹名并创建新映射；「删除全部」一键清空所有映射</li>
  <li>效果预览表支持拖拽手动排序，切换排序预设时自动清空手动顺序</li>
  <li>「压缩后校验完整性」默认关闭，开启后压缩完逐一测试压缩包</li>
  <li>压缩后重命名分「一次压缩后」「二次压缩后」两行，各自独立启用后缀替换</li>
  <li>一次压缩后的后缀替换仅在取消勾选「启用分卷」时可用，避免破坏分卷文件的 <code>.z01</code>/<code>.z02</code> 扩展名</li>
  <li>选择压缩格式时，一次/二次压缩后的默认后缀会同步更新</li>
  <li>启动时自动检查 GitHub 最新 Release，发现新版本可选下载</li>
  <li>窗口尺寸和位置自动保存恢复</li>
  <li>设置页「还原默认设置」可一键清除所有配置</li>
  <li>首次使用建议先小范围测试</li>
</ul>
"""
